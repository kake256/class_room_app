...
async function openSettings(c,maxPoints){settingsCoursework=c;settingsMaxPoints=maxPoints;const s=c.settings||{};$("settings-title").textContent=`採点基準 — ${c.title||c.id} (満点 ${maxPoints??"-"})`;fillSettingsForm(s);$("settings-error").textContent="";$("template-message").textContent="";$("settings-dialog").showModal();await loadTemplates()}
...
async function loadRanking(){const message=$("ranking-message"),head=$("ranking-head"),body=$("ranking-rows");head.replaceChildren();body.replaceChildren();if(!selectedCourseId){message.textContent="担当コースを選択してください。";return}message.textContent="ランキングを集計しています…";try{const d=await api(`/api/v1/courses/${encodeURIComponent(selectedCourseId)}/ranking`).then(r=>r.json());const tr=node("tr");for(const label of ["順位","氏名","確定点合計","確定課題数",...d.courseworks.map(c=>c.title)])tr.append(node("th",label));head.append(tr);for(const row of d.rows){const line=node("tr");for(const value of [row.rank,row.name,row.total,row.confirmed_count,...d.courseworks.map(c=>row.scores[c.coursework_id]??"—")])line.append(node("td",value));body.append(line)}message.textContent=d.rows.length?`${d.rows.length}名 / ${d.courseworks.length}課題（同点は同順位）`:"確定済み成績はまだありません。"}catch(e){message.textContent=e.message}}
...
// 答案行を表示した時刻を確認開始として保持する(点数と時刻のみ送信。答案本文は送らない)
const reviewOpenedAt=new Map();
async function saveReview(r,confirmed){const input=$(`score-${r.student_id}`),score=Number(input.value);const startedAt=reviewOpenedAt.get(r.student_id)??null;try{await api(`/api/v1/courses/${selectedCourseId}/courseworks/${selectedCourse}/reviews/${r.student_id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({score,confirmed,review_started_at:startedAt})});reviewOpenedAt.delete(r.student_id);await loadResults(selectedCourse)}catch(e){alert(e.message)}}
