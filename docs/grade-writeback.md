...
パネルのボタン(v4・3ボタンにシンプル化):

点数を確信度で2群に分けて扱う(APIの `category` 列で判定):

- **確定組 = auto_0(0点)/ auto_1(1点)/ auto_2(2点)/ auto_3(3点・システム確認済み)** …
  要確認ではない層。下書き入力→その生徒だけ選択→「返却」まで自動
- **要確認組 = candidate_3(3点候補)/ review** … 教員が目視確認する層。下書き入力のみ(返却しない)
...
| ボタン | 動作 |
|---|---|
| **全て下書き** | 未返却の全員(確定組+要確認組)に下書き入力(**返却しない**) |
| **確定のみ入力** | 確定組(auto_0/1/2/3)を下書き入力→対象生徒だけ選択→「返却」まで**完全自動**(生徒に公開・取消不可) |
| **下書き全削除** | 未返却の生徒の下書き点をまとめて空に戻す(再分析後のやり直し用) |

運用: **「全て下書き」**で全員分を下書き投入 → 成績簿を眺めて要確認組(candidate_3/review)の
判定を手直し → 手直しが済んだら **「確定のみ入力」**で確定組を自動確定 → 要確認組は目視確認して
手動で返却。**再分析(refine等)で点が変わったら「下書き全削除」→入れ直す**のが確実
(古い下書きが残らない)。

安全設計:

- **返却済み(RETURNED)の生徒には入力も削除も返却も一切触れない**(APIの `state` で判定)
- **返却は確定組(auto_0/1/2/3)だけ**。candidate_3・reviewの生徒には返却しない
...
> **少人数の課題で動作確認**し、動かなければスクリプト冒頭 `SEL` の
