...
| ボタン | 動作 |
|---|---|
| **プレビュー** | 入力予定の点数欄を青枠で色付けするだけ(入力しない) |
| **入力実行** | **未返却(TURNED_IN)の全員**にシステム点(1〜3点)を下書き入力 |
| **下書き全削除** | 未返却の生徒の下書き点をまとめて空に戻す(再分析後のやり直し用) |

運用: 全員分を下書き入力 → 成績簿を眺めて判定がおかしい生徒だけ手直し →
Classroom上で「返却」して確定。**再分析(refine等)で点が変わったら
「下書き全削除」→「入力実行」で入れ直す**のが確実(古い下書きが残らない)。

安全設計:

- **返却済み(RETURNED)の生徒には入力も削除も一切触れない**(APIの `state` で判定)
- 下書き点(draftGrade)のみ操作。**「返却」ボタンには一切触れない**
- 入力時、既に値があるセルは上書きしない(スキップ)
- クリックで開いた入力欄(activeElement)だけを操作し、別セルへの誤書き込みを防止
- 各操作後に結果を検証(入力→「成績を追加」が消えたか / 削除→戻ったか)。失敗で中断
